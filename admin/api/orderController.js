...
// ── Cancel Order with Shiprocket ──────────────────────────────────────────────
// POST /orders/:orderId/cancel  { phone }
// Public endpoint - requires phone verification, cancels order and Shiprocket shipment
const cancelOrder = async (req, res) => {
  const rawId = toStr(req.params.orderId);
  const inputDigits = toStr(req.body.phone).replace(/\D/g, '');
  ...
  try {
    ...
    // Resolve sr_cart_id → real order_id
    let orderId = Number.parseInt(rawId, 10);
    if (!Number.isFinite(orderId) || orderId <= 0 || String(orderId) !== rawId) {
    ...
    // Get order with phone and shipment details
    const [[order]] = await conn.query(
      `SELECT o.order_id, o.order_status, o.awb_code, o.shipment_id,
              MAX(ub.phone) AS billing_phone,
              MAX(us.phone) AS ship_phone
       FROM tbl_orders o
       LEFT JOIN (
         SELECT order_id, MAX(phone) AS phone
         FROM tbl_user_address WHERE address_billing = 'yes' GROUP BY order_id
       ) ub ON ub.order_id = o.order_id
       LEFT JOIN (
         SELECT order_id, MAX(phone) AS phone
    ...
    if (!phoneMatches) {
      ...
      return res.status(403).json({
        success: false,
        message: 'The mobile number does not match this order.'
      });
    }
    ...
    // Cancel Shiprocket shipment if it exists
    let shiprocketCancelled = false;
    const awbCode = toStr(order.awb_code);
    ...
    if (awbCode && shipmentId) {
      try {
        ...
        shiprocketCancelled = srRes.data?.message?.includes('success') || srRes.status === 200;
        console.log(`[cancelOrder] Shiprocket cancellation for AWB ${awbCode}:`, srRes.data);
      } catch (srErr) {
        console.error(`[cancelOrder] Shiprocket cancel failed for AWB ${awbCode}:`, srErr.response?.data || srErr.message);
