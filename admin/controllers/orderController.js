...
const { renderInvoice } = require("../api/invoiceTemplate");
...
const showOrders = async (req, res) => {
  try {
    ...
    orders.forEach(order => {
      // Normalize any uppercase CANCELED/CANCELLED to lowercase 'cancelled'
      if (order.order_status && order.order_status.toUpperCase() === 'CANCELED' || order.order_status === 'CANCELLED') {
...
const showOrder = async (req, res) => {
  try {
    ...
    for (const item of order.line_items) {
      ...
      if (productId) {
        const [[thumbMeta]] = await db.query(
          `SELECT m.media_path AS file_path
    ...
    // Also handle uppercase CANCELED/CANCELLED in order_status itself
    if (order.order_status && (order.order_status.toUpperCase() === 'CANCELED' || order.order_status === 'CANCELLED')) {
      order.order_status = 'cancelled';
    }
    if (order.shipping_status === 'CANCELLED' && order.order_status !== 'cancelled') {
...
// Function to get or create invoice number (simplified for admin)
const getOrCreateInvoiceNumber = async (orderId, orderDate) => {
  try {
    ...
    const invoiceNumber = `INV-${year}${month}-${orderId}`;
    
    // Store it
    await db.query(
      `INSERT INTO tbl_ordermeta (order_id, meta_key, meta_value) VALUES (?, '_invoice_number', ?)
    ...
    console.error('getOrCreateInvoiceNumber error:', err);
    return `INV-${orderId}`;
  }
};
...
const downloadInvoice = async (req, res) => {
  try {
    ...
    // Admin version - no phone verification needed
